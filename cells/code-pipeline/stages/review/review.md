...
- **DO** use `AskUserQuestion` for any mid-review ambiguity that blocks finding classification (see `~/.flow/runtime/kernel/run/references/user-interaction.md`). Prefer capturing ambiguities in the findings' `## Open` section over interrupting mid-review.
...
| Reference Resolver | Grep-validates references inside changed stage docs against current files. Patterns: `Step \d+(\.\d+)?` (verify the cited step still exists in the target doc), backticked section names like `` `"Conversation absorption"` `` or `` `Step 7.5` `` (verify the heading or label is still present), relative file paths like `stages/ship/ship.md` or `disciplines/tdd.md` (verify the file exists at that path), and skill names like `flow:reflect` or `kernel:run` (verify they are current — flag legacy `/flow-reflect` style). Scope: changed files in `cells/code-pipeline/stages/**` plus any stage doc transitively referenced by them; explicitly NOT `commands/*.md` (sibling thread). Output: bulleted list of broken references, each with `**file:line** — quoted reference → fix suggestion`, merged into `## Findings` by severity like the other specialists. | Broken reference to a renamed/moved stage doc = 8. Stale step number that still resolves to a real but wrong step = 7. Legacy skill name (`/flow-reflect`) = 6. Cosmetic drift (heading punctuation) = 3. |
