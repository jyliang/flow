...
- **Spike mode** — the `spike` skill overrides `AskUserQuestion` with a decision policy (pick `(Recommended)`, else first option; log to the workstream's `spike-log.md`). Stage skills remain unchanged; spike intercepts at the orchestration layer. See `spike/SKILL.md`.
