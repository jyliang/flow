...
Every stage produces a document. Every document follows this protocol.
...
**Decisions needed** — only when there are genuine choices. Omit for mechanical work.

**Verify** — only when something needs manual testing or human confirmation. Not every task needs this, but you often can't predict which tasks will. Include it when the agent isn't confident the output is correct, or when correctness depends on something outside the codebase (device behavior, third-party API, stakeholder preference).

**Revisions** — only when a later stage changes this document. This is the most important section for human-to-human communication. See "How revisions work" below.
...
Work isn't linear. During implementation you discover the spec was wrong. During review you realize the plan missed a step. When this happens:
...
2. **Add a Revisions entry** to the document you changed. The entry captures:
   - **Which stage** triggered the change (e.g., `implement → spec`)
   - **What changed** and **why** (the discovery, not just the edit)
   - **Impact** on downstream documents or work
...
2. Add a Revisions entry attributing it to the human: `**human → spec**`
