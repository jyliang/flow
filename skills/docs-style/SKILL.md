...
Each principle is a one-line rule, one good example, one bad example.
...
See `skills/flow/references/glossary.md` for the canonical term list. Extend it there (not here) when you catch drift.

## DO / DO NOT quick reference

- **DO** add a one-sentence lede to every `##` section.
- **DO** use verb-first heading names.
...
- **DO** reserve callouts (`> **Note:**`, `> **Warning:**`) for genuinely load-bearing context.
