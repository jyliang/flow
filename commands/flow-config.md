...
Ask the user these 3 questions via `AskUserQuestion`. For each, if a value is already set in the current config, present it as the `(Recommended)` option. All questions skippable — skip = keep current value (or commented default if no current value).
...
2. **Extra stages** (informational in v2, acted on in v2.5+): "Declare extra stages for this project?"
   - Header: "Extra stages"
   - No (Recommended)
   - Yes — user lists stage names

3. **Hooks dir** (informational in v2): "Declare a hooks directory for this project?"
   - Header: "Hooks"
   - No (Recommended)
