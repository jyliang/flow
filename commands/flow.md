...
If the detected stage is `explore-empty` AND config state is `configured`, your only first turn is the free-form text prompt: "What do you want to build?" — no preamble, no skill summary, no other text. This is the exception case from `references/user-interaction.md` (open-ended knowledge gathering); do NOT use `AskUserQuestion` here.

For any other detected stage, use the `flow` skill to advance work at that stage. Every decision during that advance goes through `AskUserQuestion` per the flow skill's rule.
