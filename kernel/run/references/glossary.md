...
| thread folder | thread directory, the folder, the thread | The `agent/threads/<date>-<branch>/` directory that holds one piece of work. |
...
| revision | revised version, update, new version | The `-r<N>` suffix on a thread document. `01-spec-r2.md` is the second revision. |
