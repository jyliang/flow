...
When in doubt, consult `kernel/run/references/glossary.md` for canonical term choices.
...
Pick from `kernel/run/references/glossary.md`. Never mix `spec` and `Spec` and `the spec document` in the same doc.
...
**Good:** See `kernel/run/references/protocol.md` for the full schema.

**Bad:** See kernel/run/references/protocol.md for the full schema.
...
See `kernel/run/references/glossary.md`. Extend the glossary there (not here).
...
- `kernel/ingest/ingest.md` — create or improve skills; cross-reference this doc when authoring a new skill.
- `kernel/run/references/glossary.md` — the canonical term list referenced by principle 5.
