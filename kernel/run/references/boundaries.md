...
When a later stage discovers an earlier document is wrong, revise the earlier doc rather than silently diverging. `## Revisions` is the glossary-canonical heading — it appears inside every thread doc and is defined in `kernel/run/references/protocol.md`.

1. Update the earlier document with a `## Revisions` entry (see `kernel/run/references/protocol.md`).
...
- **DO** use `AskUserQuestion` for every user-facing decision (see `kernel/run/references/user-interaction.md`).
