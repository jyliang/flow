# Review: `/flow-autopilot` spike-mode flow

**Branch**: `flow-autopilot` (diff main...HEAD)
**Date**: 2026-04-21
**Reviewer**: Claude Code

## Summary

Autopilot is a well-designed spike-mode orchestration that unattends the flow pipeline (explore → plan → implement → LLM-review → draft PR) for thesis validation. The spec, plan, and implementation are internally consistent and clear on vocabulary. Key strengths: robust abort protocol, explicit adversarial-review guidance, and safety rails around draft status. 
...
The decision policy is stated clearly in `skills/autopilot/SKILL.md:30-38` (skills/autopilot/SKILL.md:30):
- Pick `(Recommended)` option, else first
- Log to `agent/autopilot-log.md`
- Continue without pausing

And echoed in `commands/flow-autopilot.md:1` (top): "Do NOT interrupt the user at any point during this run."
...
- Autopilot does not modify those skill bodies.
- The autopilot command/skill says "when you would call AskUserQuestion, apply the decision policy instead."
...
**Vulnerability**: If the LLM invokes a stage skill directly (e.g., `/explore` or via Skill tool) without wrapping it in a context that says "you are in autopilot mode, intercept AUQ," the LLM could fall through to the normal flow and pause on the first AUQ.

**Evidence**:
- `skills/flow/references/user-interaction.md:27` correctly documents "autopilot intercepts at the orchestration layer," which is accurate but non-binding on an LLM that invokes the underlying skill bodies.
- `agent/plans/IMPLEMENTATION_PLAN_2026-04-21.md:97` acknowledges this design ("stage skill bodies have `AskUserQuestion` calls; autopilot's decision policy intercepts at the LLM level").

### Mitigations in place

1. The autopilot command explicitly states "Do NOT interrupt" (line 5).
2. `skills/autopilot/SKILL.md` is clear on the policy at the top (line 30).
3. The `commands/flow-autopilot.md` narration says "Run the explore skill with the decision policy intercepting" (line 15), which signals the LLM's role.
...
- Wrapping stage-skill invocations in a Skill-tool call that enforces autopilot mode (e.g., a pseudo-skill `_run_stage_autopilot(explore)` that blocks AUQ at the tool level).
- Or: modify stage skills to accept an `--autopilot` flag that disables AUQ (breaks the "stage skills unchanged" design principle, not recommended).
...
The abort protocol is comprehensive. From `skills/autopilot/SKILL.md:87-100`:
- Conditions: step-count ceiling, tests still failing after fix, unclear thesis, tool errors.
- Actions: commit outstanding work, open draft PR with `[AUTOPILOT ABORTED]` title, explain reason, skip Quiz/Next moves sections.
...
**Verified**: The `commands/flow-autopilot.md:25` explicitly says "still opens a draft PR, titled `[AUTOPILOT ABORTED]`," covering the no-silent-exit principle.
...
The adversarial read is mandated in `skills/autopilot/SKILL.md:64-68`:
- Four explicit internal questions before writing "What the spike shows"
- Grounded in evidence, not hypothetical

The anti-pattern section (`skills/autopilot/SKILL.md:78-84`) is **excellent**:
...
**Mitigations**:
- The PR body template (`skills/autopilot/templates/pr-body.md:25-29`) includes a comment: "MUST cover: Strongest evidence FOR, strongest evidence AGAINST, what would falsify, what skeptical reviewer would push back on." This is in the template the LLM fills, so it's a reminder at fill time.
- The Quiz section (`skills/autopilot/templates/pr-body.md:36-42`) is authored by the review stage and should probe assumptions adversarially.
...
The `autopilot-branch.sh` script handles:
...
- **Long theses**: Truncates at 40 chars slug, appends timestamp. "validate that the new approach is way faster..." → `autopilot-validate-that-the-new-approach-is-way-fa-<timestamp>`. Clean. ✓
- **Empty thesis**: Exits with code 2, usage message. ✓
- **Multiple colons/hyphens**: Normalized to single hyphens via `sed -E 's/-+/-/g'`. ✓

**Tested manually**:
```
autopilot-test-unicode-caf-r-sum-20260421-1047  ✓
autopilot-validate-that-the-new-approach-is-way-fa-20260421-1047  ✓
autopilot-hello-world-20260421-1047  ✓
(empty) → usage error  ✓
```

**Semantic observation**: Very long theses like "validate that the new approach is way faster than the old one which we should deprecate soon" get truncated to `autopilot-validate-that-the-new-approach-is-way-fa-<timestamp>`, losing semantic clarity. The timestamp ensures uniqueness, but the slug becomes less meaningful. This is acceptable tradeoff for branch names, but worth noting: **a user might kick off `/flow-autopilot` with two very similar long theses and get confusingly similar branch names**. Recommend: user responsibility (keep theses concise), not a script fix.
...
The safety rails (skills/autopilot/SKILL.md:102-110):
...
- The command and skill explicitly warn against these ("HARD rules" in commands/flow-autopilot.md:21-22).
...
### Findings: Vocabulary is tight within autopilot; spec-level clarity is good

**Risk Level**: Low

Searched for bare "review" in autopilot artifacts:

**In `skills/autopilot/SKILL.md`**:
...
**In `commands/flow-autopilot.md`**:
- Line 18: "1 LLM-review round" — qualified ✓

**In `skills/autopilot/templates/pr-body.md`**:
...
| Audit log in `agent/autopilot-log.md` | agent/plans line 14 | SKILL.md:36, templates/autopilot-log.md | ✓ |
...
- [ ] End-to-end pipeline without interrupts — not yet validated (requires live autopilot run)
...
- [ ] Parallel autopilot runs don't collide — timestamps + git isolation should handle this
...
- `skills/autopilot/SKILL.md:69` says "produce 3–5 quiz questions" after the adversarial read.
...
- `skills/autopilot/SKILL.md:108` says "refuse if `agent/spec.md` already exists."
...
Recommend: ship as-is, then monitor first few autopilot runs for unexpected AUQ pauses (suggesting the interception instruction wasn't clear enough).
